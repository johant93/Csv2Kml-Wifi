...
import MySQL.Identifiers;
import MySQL.SQLInsert;
import MySQL.SQLReading;
import MySQL.URL;
...
import filters.FilterByPosition;
...
public class Wifi_Application  {
	...
	private JTextField textaddSamp;
	...
	private void initialize() {
		...
		frameApp.getContentPane().add(panelMySQL, "name_244190410946863");
		...
		btnMysql.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				panelMySQL.setVisible(true);
				databasePanel.setVisible(false);
				System.out.println("Sql hello");
			}
		});
		btnMysql.setFont(new Font("Lucida Grande", Font.BOLD, 14));
		btnMysql.setBounds(315, 385, 122, 52);
		databasePanel.add(btnMysql);

		
		btnMenu = new JButton("Menu");
		btnMenu.setFont(new Font("Lucida Grande", Font.PLAIN, 14));
		...
		JButton btnSet = new JButton("Edit\n");

		textip = new JTextField();
		textip.setBounds(119, 143, 130, 26);
		panelMySQL.add(textip);
		textip.setColumns(10);
		
		textUser = new JTextField();
		textUser.setColumns(10);
		textUser.setBounds(119, 211, 130, 26);
		panelMySQL.add(textUser);
		
		textPassword = new JTextField();
		textPassword.setColumns(10);
		textPassword.setBounds(119, 244, 130, 26);
		panelMySQL.add(textPassword);
		
		textPort = new JTextField();
		textPort.setColumns(10);
		textPort.setBounds(119, 176, 130, 26);
		panelMySQL.add(textPort);
		
		textDb = new JTextField();
		textDb.setColumns(10);
		textDb.setBounds(484, 143, 130, 26);
		panelMySQL.add(textDb);
		
		textTable = new JTextField();
		textTable.setColumns(10);
		textTable.setBounds(484, 176, 130, 26);
		panelMySQL.add(textTable);
		
		JLabel lblNewLabel_2 = new JLabel("Ip");
		lblNewLabel_2.setBounds(46, 148, 61, 16);
		panelMySQL.add(lblNewLabel_2);
		
		JLabel lblPort = new JLabel("Port");
		lblPort.setBounds(46, 181, 61, 16);
		panelMySQL.add(lblPort);
		
		JLabel lblUser = new JLabel("User name");
		lblUser.setBounds(46, 216, 71, 16);
		panelMySQL.add(lblUser);
		
		JLabel lblPassword = new JLabel("Password");
		lblPassword.setBounds(46, 249, 61, 16);
		panelMySQL.add(lblPassword);
		
		JLabel lblDatabase = new JLabel("Database");
		lblDatabase.setBounds(418, 148, 61, 16);
		panelMySQL.add(lblDatabase);
		
		JLabel lblTable = new JLabel("Table");
		lblTable.setBounds(418, 181, 61, 16);
		panelMySQL.add(lblTable);
		
		JLabel labelTprm1 = new JLabel("Please enter your parameters :");
		labelTprm1.setFont(new Font("Lucida Grande", Font.PLAIN, 15));
		labelTprm1.setBounds(46, 95, 294, 26);
		panelMySQL.add(labelTprm1);
		
		JLabel labeltitle = new JLabel("MySQL");
		labeltitle.setHorizontalAlignment(SwingConstants.CENTER);
		labeltitle.setFont(new Font("Lucida Grande", Font.BOLD | Font.ITALIC, 20));
		labeltitle.setBounds(322, 6, 111, 52);
		panelMySQL.add(labeltitle);
		
		JLabel lblConnected = new JLabel("connected");
		lblConnected.setForeground(Color.GREEN);
		lblConnected.setFont(new Font("Lucida Grande", Font.PLAIN, 13));
		lblConnected.setVisible(false);
		lblConnected.setBounds(628, 249, 71, 16);
		panelMySQL.add(lblConnected);
		
		JButton btnConnect = new JButton("Connection to MySQL");
		btnConnect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    Sqlconnected = false;
				if(textDb.getText().isEmpty() || textTable.getText().isEmpty()) JOptionPane.showMessageDialog(null, "error : Check that all fields are correctly inputed !", "Empty Fields",2 );
				else {
				URL url = new URL(textip.getText(), textPort.getText(), textDb.getText());
				Identifiers ident = new Identifiers(url, textUser.getText(),textPassword.getText(),textTable.getText());
	           if (SQLReading.isConnected(ident)) {
					JOptionPane.showMessageDialog(null,"Connected !");	
					textDb.setEnabled(false);
	        	   ...
	        	   JOptionPane.showMessageDialog(null, "error : Connection not available. Check that all fields are correct !", "Connection",2 );
	           Sqlconnected = false ;
	           }
				}
			}
		});
		btnConnect.setFont(new Font("Lucida Grande", Font.BOLD, 15));
		btnConnect.setActionCommand("");
		btnConnect.setBounds(418, 237, 198, 40);
		panelMySQL.add(btnConnect);
		
		JLabel lblAddASample = new JLabel("Add a Sample of Wifi Scan (line) to MySQL database :");
		lblAddASample.setHorizontalAlignment(SwingConstants.LEFT);
		lblAddASample.setFont(new Font("Lucida Grande", Font.PLAIN, 15));
		lblAddASample.setBounds(46, 323, 408, 16);
		panelMySQL.add(lblAddASample);
		
		
		textaddSamp = new JTextField();
		textaddSamp.setColumns(10);
		textaddSamp.setBounds(46, 345, 555, 26);
		panelMySQL.add(textaddSamp);
		
		JButton btnAdd = new JButton("add");
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textaddSamp.getText().isEmpty())
					JOptionPane.showMessageDialog(null, "error : Check that the field is correctly inputed !", "Empty Fields",2 );
				else if (Sqlconnected){
					URL url = new URL(textip.getText(), textPort.getText(), textDb.getText());
					Identifiers ident = new Identifiers(url, textUser.getText(),textPassword.getText(),textTable.getText());
					int maxId = SQLReading.getMaxId(ident);
					WiFi_Scan ws = new WiFi_Scan(textaddSamp.getText());
					
					SQLInsert.insert_One_table(ident, maxId, ws);
				}
			else 
			 JOptionPane.showMessageDialog(null, "You're not Connected !", "Connection",2 );
			}
		});
		btnAdd.setBounds(602, 345, 117, 29);
		panelMySQL.add(btnAdd);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Sqlconnected){
					URL url = new URL(textip.getText(), textPort.getText(), textDb.getText());
					Identifiers ident = new Identifiers(url, textUser.getText(),textPassword.getText(),textTable.getText());
					db.addToDatabase(SQLReading.getWifisList(ident));
					...
					JOptionPane.showMessageDialog(null,"Succes : Database imported !");
		...
		btnNewButton.setBounds(312, 414, 142, 40);
		panelMySQL.add(btnNewButton);
		...
		btnBack_2.setBounds(6, 6, 99, 29);
		panelMySQL.add(btnBack_2);
		
		btnSet.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				textDb.setEnabled(true);
		...
		btnSet.setEnabled(false);
		btnSet.setBounds(288, 96, 71, 29);
		panelMySQL.add(btnSet);
